...
ZZCOLLAB's Docker profile system provides three ways to specify the computational environment:
...
**Scenario:** A team lead creates a new team project with specialized Docker environment.
...
**Scenario:** A team member joins a team project.
...
When additional system libraries are needed:
...
- **No manual `renv::snapshot()` required**: Automatically runs upon exiting any Docker container
...
ZZCOLLAB provides reproducible research infrastructure:
...
- **Profile range**: 14+ profiles from 200MB to 3GB
- **Reproducibility**: Dockerfile + renv.lock + .Rprofile + source code + data
