# Makefile for nof1power research compendium
# Docker-first workflow for reproducible research

PACKAGE_NAME = nof1power
R_VERSION = 4.5.1
TEAM_NAME = rgtlab
PROJECT_NAME = 
...
help:
	...
	@echo "    docker-build-plain    - Build with scrolling output (--progress=plain)"
	@echo "    docker-rebuild        - Rebuild image without cache (force fresh build)"
	@echo "    docker-build-log      - Build with detailed logs saved to file"
...
check-system-deps:
	@bash modules/validation.sh --system-deps
...
docker-build-plain:
	DOCKER_BUILDKIT=1 docker build --platform linux/amd64 --progress=plain --build-arg R_VERSION=$(R_VERSION) -t $(PACKAGE_NAME) .
...
r: check-renv
	...
	if echo "$rocker/r-ver" | grep -qE '(rocker/verse|rocker/tidyverse|rocker/rstudio|rocker/geospatial)'; then \
...
.PHONY: all document build check install vignettes test deps check-renv check-renv-no-fix check-renv-no-strict check-renv-ci docker-build docker-build-plain docker-rebuild docker-build-log docker-push-team docker-document docker-build-pkg docker-check docker-test docker-vignettes docker-render docker-rstudio docker-run r docker-check-renv docker-check-renv-fix clean docker-clean docker-disk-usage docker-prune-cache docker-prune-all help
